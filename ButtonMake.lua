...
-- @documentation https://rostrap.github.io/Libraries/Material/Button/
-- @rostrap Button
-- @author Validark
...
local function __namecall(self, ...)
	...
	return Button(Method, unpack(Arguments))
end
...
function Button.new(Type, Parent, Properties, ...)
	...
	function Metatable.Down(InputObject)
		...
		elseif ValidInputEnums[InputObject.UserInputType] then
			...
			local X, Y = InputObject.Position.X - Button.AbsolutePosition.X, InputObject.Position.Y - Button.AbsolutePosition.Y
			local V, W = X - Button.AbsoluteSize.X, Y - Button.AbsoluteSize.Y
			local a, b, c, d = (X * X + Y * Y) ^ 0.5, (X * X + W * W) ^ 0.5, (V * V + Y * Y) ^ 0.5, (V * V + W * W) ^ 0.5
			local Diameter = 2 * (a > b and a > c and a > d and a or b > c and b > d and b or c > d and c or d) + 2.5
	...
	for Property, Value in pairs(Properties) do
		if type(Property) == "number" then
			...
			__newindex(Metatable, "" .. Property, Value)
		end
	end
